...
class Block(nn.Module):
    def __init__(self,
        ...
        self.attention = Attention(n_head, d_model, 3, dropout)
    ...
    def forward(self,
        ...
        pos = pos.unsqueeze(0).expand(*key_value.size())
        output = self.attention(query, key_value, pos, slf_attn_mask)
        output *= non_pad_mask
...
class SuperBlock(nn.Module):
    def __init__(self,
        ...
        self.block = Block(n_head, d_model, dropout)
    ...
    def forward(self,
        ...
        for step, mem in zip(range(self.n_steps), memory):
            ...
            output = self.block(
                output, non_pad_mask, slf_attn_mask, mem
            )
